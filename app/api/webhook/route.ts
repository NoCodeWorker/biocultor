    ...
    if (session.payment_status === "paid") {
      ...
      try {
        ...
        const customerEmail =
          ...
          `anon-${session.id}@biocultor.local`
        ...
        if (pendingCartId) {
          ...
          if (pending) {
            ...
            console.warn(
              `PendingCart ${pendingCartId} no encontrado (¿ya consumido y purgado?).`
            )
          }
        ...
        try {
          let crmContact = await prisma.crmContact.findFirst({
            where: {
              OR: [{ email: customerEmail }, { customerId: customer.id }],
            },
          })
        ...
        if (pendingCartId) {
          prisma.pendingCart
            .deleteMany({ where: { id: pendingCartId } })
            .catch(() => {})
        }
        ...
        try {
          const firstItem = metaCartItems[0]
          const firstVariant = firstItem
            ? variantRows.find((variant) => variant.id === firstItem.id)
            : null

          await prisma.ecommerceEvent.create({
            data: {
              eventName: "purchase",
              sessionId: pendingCartId || session.id,
              productSlug: firstVariant?.product.slug ?? null,
              variantSku: firstVariant?.sku ?? null,
              variantSize: firstVariant?.size ?? null,
              itemName: firstVariant?.product.name ?? null,
              quantity: firstItem?.q ?? null,
              value: (session.amount_total || 0) / 100,
              currency: "EUR",
              device: "unknown",
              sourcePath: "/api/webhook",
              orderNumber: createdOrder.orderNumber,
              stripeSession: session.id,
              dedupeKey: `purchase:${session.id}`,
              metadataJson: JSON.stringify({
                pendingCartId,
                items: metaCartItems.map((item) => {
                  const variant = variantRows.find((row) => row.id === item.id)
                  return {
                    id: item.id,
                    sku: variant?.sku ?? null,
                    productSlug: variant?.product.slug ?? null,
                    name: variant?.product.name ?? "Producto Biocultor",
                    size: variant?.size ?? "",
                    quantity: item.q,
                    unitPrice: item.p,
                  }
          ...
          const code = (analyticsError as { code?: string } | null)?.code
          if (code !== "P2002") {
            console.error(
              "Error registrando evento purchase analytics:",
              analyticsError
            )
          }
        }
        ...
        try {
          const { sendOrderConfirmationEmail, sendAdminOrderNotification } =
          ...
          console.error(
            "Error enviando correos transaccionales (no se reintenta el webhook):",
            emailErr
          )
        }
      } catch (dbError) {

...
RUN node_modules/.bin/esbuild scripts/inject-legacy-posts-alts.ts \
      --bundle \
      --platform=node \
      --packages=external \
      --outfile=inject-legacy-posts-alts.cjs

RUN node_modules/.bin/esbuild scripts/seed-service-application-posts.ts \
      --bundle \
      --platform=node \
      --packages=external \
